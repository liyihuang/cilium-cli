...
func initSysdumpFlags(cmd *cobra.Command, options *sysdump.Options, optionPrefix string, hooks SysdumpHooks) {
	...
	cmd.Flags().StringVar(&options.CiliumHelmChartReleaseName,
		"cilium-helm-chart-release-name", sysdump.DefaultCiliumHelmChartReleaseName,
		"Cilium Helm chart release name to get the values from the sysdump")
